...
export const onRequest: PagesFunction<{
  ...
  if (path === '/api/lab/start-new' && request.method === 'POST') {
    try {
      ...
      const userId = await validateSession(env, request);
      if (!userId) return errorResponse('Invalid session', 401);
      const csrf_token = (await request.formData()).get('csrf_token')?.toString();
    ...
    try {
      ...
      for (const key of userKeys.keys) {
        try {
          ...
          await logActivity(env, userId, 'user_key_error', { ip, key: key.name, error: e.message });
          continue;
        }
      }
      ...
      await logActivity('admin_users_progress', 's', { ip, userId: currentUser.id });
...
async function getUserById(env: { USERS: KVNamespace }, userId: string): Promise<User | null> {
  const userKeys = await env.USERS.list();
  for (const key of userKeys.keys) {
    const userData = await env.USERS.get(key.name);
    if (userData) {
