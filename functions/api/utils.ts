...
export async function onRequest(context) {
  ...
  // Helper to return JSON responses (imported from utils.ts, included for reference)
  ...
  if (path === '/api/avatar' && request.method === 'POST') {
    try {
      ...
      if (!await checkRateLimit(env, `avatar:${clientIP}`, 5, 3600)) {
      ...
      // Log activity using utility function
      await logActivity(env, userId, 'avatar_upload', { size: avatar.size });
  ...
  if (path === '/api/change-password' && request.method === 'POST') {
    try {
      ...
      // Note: In production, use proper password verification (e.g., bcrypt)
      ...
      // Log activity
      await logActivity(env, userId, 'password_change');
  ...
  if (path.startsWith('/api/user-avatar/') && request.method === 'GET') {
    try {
      ...
      const requestingUser = await getUserById(env, requestingUserId);
